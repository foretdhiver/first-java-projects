...
<html>
	<head>
		...
		//		alert("r_arr[2] : " + r_arr[2]);
			...
			$('#eaapprove').click(function(){
				var eastatus = $('#ea_status').val();
				if(confirm('해당 문서를 승인 하시겠습니까?')){
					loading();
					$('#eadetailform').attr({
						"method":"POST",
						"enctype":"multipart/form-data",
						"action":"/ea/eaapproval.ge"
					});
					$('#eadetailform').submit();
				}else{
				}
			...
			$('#eareject').click(function(){
				var eastatus = $('#ea_status').val();
				if(confirm('해당 문서를 반려 하시겠습니까?')){
					loading();
					$('#eadetailform').attr({
						"method":"POST",
						"enctype":"multipart/form-data",
						"action":"/ea/eareject.ge"
					});
					$('#eadetailform').submit();
				}else{	
				}
			...
			$('#eastand').click(function(){
				var eastatus = $('#ea_status').val();
				if(confirm('해당 문서를 보류 하시겠습니까?')){
					loading();
					$('#eadetailform').attr({
						"method":"POST",
						"enctype":"multipart/form-data",
						"action":"/ea/eastand.ge"
					});
					$('#eadetailform').submit();
				}else{
				}
			...
			/* 결재업로드 파일 업로드 하지 않을 시 버튼이 안눌리는 validation 만들기 */
		});
			...
			<form name="ge_main" id="ge_main">
				...
				<div class="outertable">
					...
					<div class="ge_sidemenu">
						...
						<a href="/ea/eamyapplist.ge" style="color:#615F8D">결재요청문서</a><br>
						</div>
...
%>		
		...
		<form id="eadetailform" name="eadetailform">
		...
		<table id="outlinetable" class="eawritetable">
			...
			<tr>
				<td id="eabutton" colspan="2" align="center">
					<input type="button" class="button" id="eaapprove" name="eaapprove" value="승인">
					<input type="button" class="button" id="eareject" name="eareject" value="반려" >	
					<input type="button" class="button" id="eastand" name="eastand" value="대기" >
				</td>
			</tr>
		</table>
		</div>
