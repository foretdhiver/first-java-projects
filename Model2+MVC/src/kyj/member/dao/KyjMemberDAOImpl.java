...
import kyj.member.common.KyjChaeBunClass;
import kyj.member.sql.KyjSqlQueryMap;
import kyj.member.vo.KyjMemberVO;
import kyj.member.common.KyjConnProperty;
...
public class KyjMemberDAOImpl implements KyjMemberDAO {
	...
	public boolean insertKyjMember(KyjMemberVO _kvo) {
		...
		try{
			...
			pstmt.setString(10, _kvo.getKimage());
			pstmt.setString(11, _kvo.getKjuso1());
	...
	public boolean updateKyjMember(KyjMemberVO _kvo) {
		...
		try{
			...
			pstmt.setString(3, _kvo.getKhp());
			pstmt.setString(4, _kvo.getKbirth());
			pstmt.setString(5, _kvo.getKemail());
			pstmt.setString(6, _kvo.getKpostno());
			pstmt.setString(7, _kvo.getKjuso());
			pstmt.setString(8, _kvo.getKjuso1());
			pstmt.setString(9, _kvo.getKimage());
			pstmt.setString(10, _kvo.getKid());
			nCnt = pstmt.executeUpdate();
	...
	public boolean deleteKyjMember(KyjMemberVO _kvo) {
		...
		System.out.println("(log) _kvo 참조변수 주소값 : " + _kvo);
		System.out.println("dfdfdfdf >>> : " + _kvo.getKid());
		Connection con = null;
		PreparedStatement pstmt = null;
		int nCnt = 0;
...
//			System.out.println("(log) rsRs.next() >>> : " + rsRs.next());
			if(rsRs!=null){
				...
				while(rsRs.next()){
					...
					kvo.setKjuso1(rsRs.getString("KJUSO1"));
					...
					kvo.setKimage(rsRs.getString("KIMAGE"));
					aList.add(kvo);
	...
	public ArrayList<KyjMemberVO> searchKyjMember(KyjMemberVO _kvo) {
		...
		try{
			...
			pstmt.setString(1, _kvo.getKid());
			System.out.println("(log) _kvo >>> : " + _kvo.getKid());
			rsRs = pstmt.executeQuery();
			System.out.println("(log) rsRs >>> : " + rsRs);
...
//					
	...
	// 로그인
		...
		@Override
		public boolean updateKyjMember1(KyjMemberVO _kvo) {
			// TODO Auto-generated method stub
			System.out.println("(log) KyjMemberDAOImpl.updateKyjMember() 시작 >>> " );
			System.out.println("(log) _kvo 참조변수 주소값 : " + _kvo);
			...
			try{
				con = KyjConnProperty.getConnection();
				System.out.println("(log) con >>> : " + con);
				pstmt = con.prepareStatement(KyjSqlQueryMap.getUpdateQuery1());
				System.out.println("(log) pstmt >>> : " + pstmt);
				System.out.println("\n(log) KyjSqlQueryMap.getUpdateQuery1() >>> 쿼리문 확인 ");
				System.out.println(KyjSqlQueryMap.getUpdateQuery1());
				pstmt.setString(1, _kvo.getKpw());
				pstmt.setString(2, _kvo.getKname());
				pstmt.setString(3, _kvo.getKhp());
				pstmt.setString(4, _kvo.getKbirth());
		...
		@Override
		public int idValueCkeck(KyjMemberVO _kvo) {
			// TODO Auto-generated method stub
			System.out.println("(log) KyjMemberDAOImpl.idValueCkeck() 시작 >>> " );
			System.out.println("(log) _kvo 참조변수 주소값 : " + _kvo);
			Connection con = null;
			PreparedStatement pstmt = null;
			int nCnt = 0;
			...
			if(nCnt==0){
				try{
					con = KyjConnProperty.getConnection();
					System.out.println("(log) con >>> : " + con);
					pstmt = con.prepareStatement(KyjSqlQueryMap.idValueCkeck());
					System.out.println("(log) pstmt >>> : " + pstmt);
					System.out.println("\n(log) KyjSqlQueryMap.idValueCkeck() >>> 쿼리문 확인 ");
					System.out.println(KyjSqlQueryMap.idValueCkeck());
					pstmt.setString(1, _kvo.getKid());
					nCnt = pstmt.executeUpdate();
			...
			System.out.println("(log) KyjMemberDAOImpl.idValueCkeck() 끝 >>> " );
			return nCnt;
